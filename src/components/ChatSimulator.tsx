...
export const ChatSimulator: React.FC<ChatSimulatorProps> = ({
  ...
  return (
    <div className="relative flex flex-col h-[calc(100vh-140px)] min-h-[520px] bg-slate-950 rounded-2xl border border-slate-800 shadow-2xl overflow-hidden">
      ...
      {chatMode === 'global' && activeDuel && activeDuel.status !== 'idle' && (
        <LiveDuelPanel
          duel={activeDuel}
          currentUserId={currentUserId}
          compact={!(activeDuel.player1.id === currentUserId || activeDuel.player2.id === currentUserId)}
        />
      )}
          ...
          <div className="flex-1 flex flex-col bg-slate-950/80 overflow-y-auto p-4 space-y-3 scrollbar-thin">
            {globalMessages.map((msg) => {
              ...
              const isDev =
                ...
                msg.senderId === '#1' ||
                (msg.senderName && (msg.senderName.toLowerCase() === 'shiro anna' || msg.senderName.toLowerCase().includes('shiro')));
                ...
                <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} space-y-1`}>
                  <div className="flex items-center space-x-2 px-1">
                    <span className="text-[11px] font-bold text-slate-300 flex items-center gap-1.5">
                      ...
                      <span className={isDev ? "dev-rainbow-text font-extrabold" : ""}>{msg.senderName}</span>
                      {isDev && (
                      ...
                      <div className="flex items-center space-x-2.5 overflow-hidden">
                        ...
                        <div className="overflow-hidden">
                          <div className="flex items-center space-x-1">
                            <span className={`text-xs font-bold truncate ${isDev ? 'dev-rainbow-text' : 'text-white'}`}>{friend.username}</span>
                            {isDev && (
                ...
                <>
                  ...
                  <div className="p-3 bg-slate-900/90 border-b border-slate-800 flex items-center justify-between">
                    <div className="flex items-center space-x-2.5">
                      ...
                      />
                      <div>
                        <h4 className={`text-xs font-bold ${selectedFriend.id === '#1' ? 'dev-rainbow-text' : 'text-white'}`}>{selectedFriend.username}</h4>
          ...
          <div className="flex-1 flex flex-col bg-slate-950/80 overflow-y-auto p-4 space-y-3 scrollbar-thin">
            {messages.map((msg) => {
              ...
              const isUserDev =
                ...
                currentUserId === '#1' ||
                (userName && (userName.toLowerCase() === 'shiro anna' || userName.toLowerCase().includes('shiro')));
              const isBotDev =
                botProfile.role === 'Developer' ||
                (botProfile.name && botProfile.name.toLowerCase().includes('shiro'));
              const isDev = isUser ? isUserDev : isBotDev;

              return (
                <div key={msg.id} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} space-y-1`}>
                  <div className="flex items-center space-x-2 px-1">
                    {!isUser && (
                      <BotAvatar src={botProfile.avatar} alt={botProfile.name} className="w-5 h-5" />
                    )}
                    <span className={isDev ? "dev-rainbow-text font-extrabold text-[11px]" : "text-[10px] font-medium text-slate-400"}>
                      {isUser ? (msg.senderName || userName) : botProfile.name}
                    </span>
                    {isDev && (
                      <span className="text-[9px] bg-rose-500/20 text-rose-300 border border-rose-500/40 px-1.5 py-0.2 rounded font-black">
                        DEV
                  ...
                  <div
                    className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm whitespace-pre-wrap leading-relaxed shadow-lg ${
                      isUser
                        ? isUserDev
                          ? 'bg-slate-900 border border-rose-500/40 text-rose-100 rounded-br-none shadow-rose-950/50'
                          : 'bg-sky-600 text-white rounded-br-none'
                        : 'bg-slate-900 text-slate-100 rounded-bl-none border border-slate-800'
                    }`}
                  >
