...
// ==========================================
// HELPER FUNCTIONS (DI LUAR KOMPONEN UTAMA)
// ==========================================
...
export const MediaNotificationOverlay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const latestPointer = { clientX: 0, clientY: 0 };

    const getNativeRAF = () => (window as any)._nativeRAF || window.requestAnimationFrame.bind(window);
    const getNativeCancel = () => (window as any)._nativeCancel || window.cancelAnimationFrame.bind(window);

    const handlePointerMove = (e: PointerEvent) => {
      latestPointer.clientX = e.clientX;
      latestPointer.clientY = e.clientY;
  ...
  const getCurrentTrackSourceUrl = () => {
    if (!currentTrack) return '';
    if (currentTrack.sourceUrl) return currentTrack.sourceUrl;

    if (currentTrack.source === 'spotify') {
      return currentTrack.videoId || currentTrack.trackId || '';
    }

    const rawVideoId = (currentTrack.videoId || '').replace(/^yt_/, '');
    if (rawVideoId && /^https?:\/\//i.test(rawVideoId)) {
      return rawVideoId;
    }
    return rawVideoId ? `https://www.youtube.com/watch?v=${rawVideoId}` : '';
  };
  ...
  const downloadCurrentTrack = async () => {
    ...
    try {
      let audioUrl = isDirectAudioUrl(currentTrack.downloadUrl) ? currentTrack.downloadUrl : '';

      if (!audioUrl) {
        const sourceUrl = getCurrentTrackSourceUrl();
        if (!sourceUrl) {
      ...
      const safeName = (currentTrack.title || 'audio').replace(/[\/:*?"<>|]+/g, '_').trim() || 'audio';
      ...
      console.warn('[AUDIO DOWNLOAD WARN] Failed to download current track:', err);
    }
  };
  ...
  return (
    <div
      style={{
        ...
        transition: isDragging || isResizing ? 'none' : 'opacity 0.3s ease-out, transform 0.15s ease-out',
      }}
            ...
            <div className="flex-1 overflow-hidden flex flex-col justify-between h-full">
              ...
              <div className="flex items-center space-x-1.5 mt-2 no-drag">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg bg-sky-600/20 hover:bg-sky-600 text-sky-300 hover:text-white border border-sky-500/30 shadow transition-all active:scale-95"
