...
const SPECTATOR_COLLAPSE_MS = 4000;
const SPECTATOR_HOLD_MS = 800;
const SPECTATOR_FADE_OUT_MS = 2000;
...
export const LiveDuelPanel: React.FC<LiveDuelPanelProps> = ({ duel, currentUserId, compact = false }) => {
  const isParticipant = duel.player1.id === currentUserId || duel.player2.id === currentUserId;
  const isSpectator = !isParticipant;
  const isTimeout = duel.status === 'timeout';
  const [shouldShowCompact, setShouldShowCompact] = useState(true);
  const [compactExiting, setCompactExiting] = useState(false);

  useEffect(() => {
    if (!compact) {
      setShouldShowCompact(true);
      setCompactExiting(false);
      return;
    }

    const hideWhenClosed = duel.status === 'timeout' || duel.status === 'finished';
    if (hideWhenClosed) {
      setCompactExiting(true);
      const hideTimer = setTimeout(() => {
        setShouldShowCompact(false);
      }, 1000);
    ...
    setShouldShowCompact(true);
    setCompactExiting(false);
  }, [compact, duel.id, duel.status, duel.updatedAt]);

  if (compact && !shouldShowCompact) return null;

  const duelTitle = `${duel.player1.name} Vs ${duel.player2.name}`;

  if (compact) {
    return (
      <div className={`absolute inset-x-3 sm:inset-x-8 top-2 z-30 max-w-2xl mx-auto pointer-events-none ${compactExiting ? 'animate-duel-spectator-exit' : 'animate-duel-spectator-fade'}`}>
        <div className="relative overflow-hidden bg-slate-900/90 backdrop-blur-md border border-amber-500/50 rounded-2xl px-4 py-2.5 text-slate-100 shadow-2xl">
          <div className="absolute inset-0 bg-gradient-to-r from-amber-500/10 via-sky-500/10 to-indigo-500/10" />
          <div className="relative flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-7 h-7 rounded-full bg-amber-500/20 border border-amber-400 flex items-center justify-center text-amber-300 flex-shrink-0">
                <Swords className="w-3.5 h-3.5" />
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="text-xs font-black text-amber-300 tracking-wide uppercase truncate">
                    LIVE DUEL ARENA
                  </h3>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-500/20 text-amber-200 border border-amber-500/40">
                    {duel.status === 'timeout' ? 'WAKTU HABIS' : `Ronde ${duel.currentRound} / ${duel.totalRounds}`}
                  </span>
                </div>
  ...
  return (
    <div className={`absolute inset-x-3 sm:inset-x-8 top-12 z-30 max-w-md mx-auto ${isTimeout ? 'animate-duel-timeout-exit' : 'animate-duel-panel-fade'}`}>
      ...
      <div className="relative bg-slate-900/90 backdrop-blur-md border border-amber-500/50 rounded-2xl p-4 sm:p-5 text-slate-100 shadow-2xl transition-opacity duration-300 hover:opacity-100 opacity-95">
        ...
        {/* 5. TIMEOUT STEP */}
        {duel.status === 'timeout' && (
          <div className="text-center py-4 bg-gradient-to-b from-rose-950/85 to-slate-950 rounded-xl border border-rose-400/50 space-y-2">
            <Clock className="w-10 h-10 text-rose-300 mx-auto animate-pulse" />
            <h4 className="text-base font-extrabold text-rose-200">
              WAKTU HABIS!
            </h4>
            <p className="text-xs text-slate-300">
              {duel.lastAnswerText || 'Tidak ada respons selama 1 menit. Ronde duel ditutup.'}
            </p>
            <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-rose-500/20 border border-rose-400/40 text-rose-200 text-xs font-bold">
              <ShieldCheck className="w-3.5 h-3.5" />
              <span>Panel akan hilang sebentar lagi...</span>
            </div>
          </div>
        )}
