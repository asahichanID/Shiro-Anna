...
import { Trophy, Coins, Zap, HelpCircle, Gamepad2, Database, ShieldCheck, Music, Sparkles, Wrench, MoreVertical, Headset, ShoppingBag, Settings, Tv } from 'lucide-react';
...
export type HeaderTab = 'chat' | 'nobar' | 'play' | 'shop' | 'service' | 'devpanel' | 'database' | 'queue' | 'analysis' | 'profile';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          ...
          <div className="flex items-center gap-3 overflow-x-auto pb-1 md:pb-0">
            ...
            >
              ...
              <div className="flex items-center space-x-1.5 text-xs font-medium">
                <span className={isDeveloper ? "font-extrabold dev-rainbow-text" : "text-slate-200 font-bold"}>
            ...
            <button
              ...
              className="p-2 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 hover:text-white transition-all cursor-pointer flex items-center justify-center shadow-sm"
              title="Pengaturan & Service Hub"
            >
              <Settings className="w-4 h-4 text-amber-400 hover:rotate-45 transition-transform duration-300" />
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-3 pt-2 border-t border-slate-800 text-xs font-medium">
          <div className="flex items-center space-x-2 overflow-x-auto pb-1 sm:pb-0">
            {/* Square Oguri Chat Button */}
            <button
              onClick={() => setActiveTab('chat')}
              title="Oguri Chat (Chat Bot Simulator)"
              className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all flex-shrink-0 cursor-pointer border ${
                activeTab === 'chat'
                  ? 'bg-sky-600 text-white font-bold shadow-md shadow-sky-600/30 border-sky-400 scale-105'
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-3.5 py-1.5 rounded-lg transition-all flex-shrink-0 cursor-pointer ${
                activeTab === 'nobar'
                  ? 'bg-gradient-to-r from-indigo-600 to-sky-600 text-white font-extrabold shadow-md shadow-indigo-600/30 border border-indigo-400 scale-105'
                  : 'bg-indigo-950/40 text-indigo-300 hover:text-white hover:bg-indigo-900/60 border border-indigo-500/30 font-bold'
              }`}
            >
