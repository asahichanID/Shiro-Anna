...
export const ShopView: React.FC = () => {
  ...
  const [dialogMode, setDialogMode] = useState<'none' | 'insufficient' | 'confirm_premium' | 'confirm_badge' | 'alert' | 'success_badge'>('none');
  ...
  const loadShopData = async () => {
    try {
      const [prods, ords, cHist, userBadgesRes] = await Promise.all([
              ...
              return (
                ...
                >
                  ...
                  <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between gap-3">
                    ...
                    >
                      {isOwned ? 'Telah Dimiliki' : 'Beli Badge'}
                    </button>
                  </div>
                </div>
              );
