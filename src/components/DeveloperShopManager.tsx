        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-5">
          ...
          <div className="space-y-2 rounded-2xl border border-amber-500/20 bg-amber-950/20 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h5 className="text-xs font-extrabold text-amber-200 uppercase tracking-wider">
                  Mode Maintenance Shop
                </h5>
                <p className="text-[11px] text-slate-400">
                  Toggle ini tersimpan di D1, jadi shop utama bisa langsung berubah tanpa ubah tombol di dalam shop.
                </p>
              </div>
              <button
                onClick={() =>
                  setShopConfig({
                    ...
                    shop_maintenance_enabled:
                      (shopConfig.shop_maintenance_enabled || 'false') === 'true' ? 'false' : 'true',
                  })
                }
                className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${
                  (shopConfig.shop_maintenance_enabled || 'false') === 'true'
                    ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40'
                    : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40'
                }`}
              >
                {(shopConfig.shop_maintenance_enabled || 'false') === 'true' ? 'TRUE' : 'FALSE'}
              </button>
            </div>
          </div>

          <div className="space-y-2 rounded-2xl border border-emerald-500/20 bg-emerald-950/20 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h5 className="text-xs font-extrabold text-emerald-300 uppercase tracking-wider">
                  Mode Pemrosesan Manual WA (Proses Cepat)
                </h5>
                <p className="text-[11px] text-slate-400">
                  Jika ON, tombol 'Proses Cepat via WhatsApp' (wa.me/6281563808289) aktif untuk konfirmasi pesanan Premium Wibuku.
                </p>
              </div>
              <button
                onClick={() =>
                  setShopConfig({
                    ...shopConfig,
                    shop_manual_mode_enabled:
                      (shopConfig.shop_manual_mode_enabled || 'true') === 'true' ? 'false' : 'true',
                  })
                }
                className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${
                  (shopConfig.shop_manual_mode_enabled || 'true') === 'true'
                    ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40'
                    : 'bg-slate-800 text-slate-400 border border-slate-700'
                }`}
              >
                {(shopConfig.shop_manual_mode_enabled || 'true') === 'true' ? 'ON (TRUE)' : 'OFF (FALSE)'}
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            ...
            <div className="space-y-1.5 md:col-span-2">
              <label className="block font-bold text-slate-200">
                Pesan Maintenance Shop
              </label>
              <input
                type="text"
                value={shopConfig.shop_maintenance_message || ''}
                onChange={(e) => setShopConfig({ ...shopConfig, shop_maintenance_message: e.target.value })}
                className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-700 text-white focus:outline-none focus:border-amber-500"
              />
            </div>
