  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fadeIn pb-12">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
        ...
        {/* Settings Button (⚙️) - Top Right Corner following Profile Card Border */}
        <button
          onClick={() => setIsSettingsOpen(true)}
          className="absolute top-3 right-3 sm:top-4 sm:right-4 z-20 p-2.5 rounded-xl bg-slate-800/80 hover:bg-slate-700/90 text-slate-300 hover:text-amber-400 border border-slate-700/80 shadow-md backdrop-blur-xs transition-all cursor-pointer group hover:scale-105 active:scale-95"
          title="Pengaturan & Service Hub"
        >
          <Settings className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400 group-hover:rotate-45 transition-transform duration-300" />
        </button>
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2.5">
                ...
                <span className="text-xs font-bold px-2.5 py-1 rounded-md bg-slate-800/90 text-slate-300 border border-slate-700 flex items-center gap-1">
                  ...
                  ID: {isDeveloper || profile.id === '#1' || profile.publicId === '#1' ? '#1' : `#${(profile.publicId || profile.accountCode || '').replace(/^#/, '')}`}
                </span>
              ...
              {isEditingName ? (
                ...
                <div className="flex items-center justify-center md:justify-start space-x-3 pt-1">
                  <h2
                    className={
                      isDeveloper
                        ? 'text-2xl sm:text-3xl font-extrabold tracking-tight dev-rainbow-text'
