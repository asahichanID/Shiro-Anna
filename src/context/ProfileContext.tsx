...
const ensureSessionMeta = (account: UserAccount): UserAccount => {
  const isDev = account.id === '#1' || account.role === 'Developer' || account.username.toLowerCase() === 'shiro anna';
  const effectivePublicId = isDev ? '#1' : (account.publicId || account.accountCode || generateSevenDigitPublicId());
  return {
    ...account,
    publicId: effectivePublicId,
    accountCode: effectivePublicId,
    ...
    lastOnline: account.lastOnline || Date.now(),
...
export const ProfileProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = (inputName: string): { success: boolean; error?: string } => {
    ...
    if (isDevName) {
      ...
      const existingPublicIds = Object.values(accountsMap)
        .flatMap((a) => [a.publicId, a.accountCode])
        .filter(Boolean) as string[];
      const nextPublicId = generateSevenDigitPublicId(existingPublicIds);
