...
// ==========================================
// HELPER FUNCTIONS (Mencegah Error Not Defined)
// ==========================================
...
export const AudioPlayerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (!audioRef.current) {
      ...
      audio.preload = 'auto';
      audio.volume = 0.4; // 💡 Set volume awal audio jadi 40%
  ...
  useEffect(() => {
    ...
    if ('mediaSession' in navigator && typeof MediaMetadata !== 'undefined') {
      navigator.mediaSession.metadata = new MediaMetadata({
        title: currentTrack.title || 'Judul Lagu',
        artist: currentTrack.artist || 'Oguri Cap 🐎',
        album: 'Oguri Jukebox Native',
        artwork: [
          {
            src: currentTrack.thumbnail || 'https://via.placeholder.com/512',
            sizes: '512x512',
            type: 'image/jpeg',
          },
        ],
  ...
  const playTrack = async (track: JukeboxTrack, newPlaylist?: JukeboxTrack[]) => {
    ...
    const hasPlayableAudioUrl = isDirectAudioUrl(activeTrack.downloadUrl);

    if (!hasPlayableAudioUrl || isExpired) {
      ...
      if (vId) {
        try {
          ...
          const freshUrl =
            refreshRes.success &&
            refreshRes.result &&
            (refreshRes.result.download ||
              refreshRes.result.url ||
              refreshRes.result.link ||
              refreshRes.result.audio ||
              refreshRes.result.mp3 ||
              '');
    ...
    if (!isDirectAudioUrl(activeTrack.downloadUrl)) {
